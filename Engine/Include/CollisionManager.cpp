...
bool CCollisionManager::CreateProfile(const std::string& Name, const std::string& ChannelName, bool Enable, ECollision_Interaction BaseInteraction)
{
	...
	for (int y = (int)OverlapInfo.Top; y < (int)OverlapInfo.Bottom; y++)
	{
		for (int x = (int)OverlapInfo.Left; x < (int)OverlapInfo.Right; x++)
		{
			...
			case EPixelCollision_Type::Color_Ignore:
				if (Dest.Pixel[Index] == Dest.TypeColor[0] && Dest.Pixel[Index + 1] == Dest.TypeColor[1] && Dest.Pixel[Index + 2] == Dest.TypeColor[2])
				{
					continue;
				}
