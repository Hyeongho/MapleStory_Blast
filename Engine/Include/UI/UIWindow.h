...
protected:
    ...
    std::vector<std::shared_ptr<CUIWidget>>	m_vecWidget;
    int m_ZOrder;
	Vector2	m_Pos;
	Vector2	m_Size;
	bool m_Start;
...
private:
	static bool SortWidget(std::shared_ptr<CUIWidget> Src, std::shared_ptr<CUIWidget> Dest);
	static bool SortWidgetInv(std::shared_ptr<CUIWidget> Src, std::shared_ptr<CUIWidget> Dest);
