...
CInput::CInput() : m_MouseLDown(false), m_MouseLPush(false), m_MouseLUp(false), m_InputSystem(InputSystem_Type::DInput), m_Input(nullptr), m_Keyboard(nullptr), m_Mouse(nullptr), m_KeyArray{}, m_MouseState{}, m_CollisionWidget(false)
{
}
...
void CInput::UpdateBindKey(float DeltaTime)
{
	...
	for (; iter != iterEnd; ++iter)
	{
		if (iter->second->key->Down && iter->second->Ctrl == m_Ctrl && iter->second->Alt == m_Alt && iter->second->Shift == m_Shift)
		{
		...
		if (iter->second->key->Push && iter->second->Ctrl == m_Ctrl && iter->second->Alt == m_Alt && iter->second->Shift == m_Shift)
		{
		...
		if (iter->second->key->Up && iter->second->Ctrl == m_Ctrl && iter->second->Alt == m_Alt && iter->second->Shift == m_Shift)
		{
