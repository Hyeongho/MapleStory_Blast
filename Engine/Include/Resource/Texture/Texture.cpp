...
CTexture::CTexture() : m_Scene(nullptr), m_ArraySRV(nullptr), m_ImageType(EImageType::Atlas)
{
	SetTypeID<CTexture>();
}
...
bool CTexture::CreateResourceArray()
{
	...
	if (FAILED(DirectX::CreateTextureEx(CDevice::GetInst()->GetDevice(), ImageArray->GetImages(), ImageArray->GetImageCount(), ImageArray->GetMetadata(), D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0, DirectX::CREATETEX_FLAGS::CREATETEX_DEFAULT,
