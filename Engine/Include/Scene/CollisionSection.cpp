...
void CCollisionSection::Collision(float DeltaTime)
{
	...
	for (; iter != iterEnd; iter++)
	{
		...
		for (; iter1 != iter1End; iter1++)
		{
			...
			if (SrcProfile->vecCollisionInteraction[(int)DestProfile->Channel->Channel] == ECollision_Interaction::Ignore || DestProfile->vecCollisionInteraction[(int)SrcProfile->Channel->Channel] == ECollision_Interaction::Ignore)
			{
				continue;
			}
...
CCollider* CCollisionSection::CollisionMouse(const Vector2& MouseWorldPos, float DeltaTime)
{
	...
	{
		m_ColliderList.sort(CCollisionSection::ColliderSort);
	}
...
bool CCollisionSection::ColliderSort(CCollider2D* Src, CCollider2D* Dest)
{
