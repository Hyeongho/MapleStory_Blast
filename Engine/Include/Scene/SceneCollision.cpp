...
CSceneCollision::CSceneCollision() : m_CollisionWidget(false)
{
}
...
{
	for (int i = 0; i < m_Section.Count; ++i)
	{
		m_Section.vecSection[i]->m_ColliderList.clear();
	}
...
void CSceneCollision::CreateSection2D(int CountX, int CountY, const Vector2& WorldStart, const Vector2& SectionSize)
{
	...
	m_Section.Count = CountX * CountY;
	m_Section.WorldStart = WorldStart;
	m_Section.SectionSize = SectionSize;
	m_Section.WorldSize = SectionSize * Vector2((float)CountX, (float)CountY);
...
void CSceneCollision::CollisionMouse(float DeltaTime)
{
	...
	if (!m_CollisionWidget)
	{
		...
		MouseIndex -= m_Section.WorldStart;
...
bool CSceneCollision::CreatePixelCollisionFullPath(const std::string& Name, const TCHAR* FullPath)
{
	...
	if (FAILED(DirectX::CreateShaderResourceView(CDevice::GetInst()->GetDevice(), Image->GetImages(), Image->GetImageCount(), Image->GetMetadata(), &Info->SRV)))
	{
		return false;
	}
